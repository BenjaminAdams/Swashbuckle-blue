...
SwaggerUi.Views.OperationView = Backbone.View.extend({
	...
	events: {
		...
		'click .copyResponseBodyTxt': 'copyResponseBodyTxt'
	},
	...
	render: function() {
			...
			for(key in successResponse) {
				...
				if(typeof value === 'object' && typeof value.createJSONSample === 'function') {
					...
					signatureModel = {
						...
						//signature2: value.getMockSignature()
					};
				}
			}
		...
		//injected the custom model description here
		if(responseSignatureHtml !== '') {
			this.model.responseSignatureHtml = responseSignatureHtml
		}

		if(inputSignatureHtml !== '') {
			this.model.inputSignatureHtml = inputSignatureHtml
		}
	...
	copyResponseBodyTxt: function(e) {
		e.preventDefault()
		e.stopPropagation()
		var target = $(e.currentTarget);
		//var txtResponse = $('.response_body code', $(this.el));
		//console.log('txtResponse=', txtResponse)
		var txtFromTxtBody = this.responsetxt
	...
	showStatus: function(response) {
		...
		this.responsetxt = jQuery.trim(content);
		...
		if(!content) {
			...
			try {
				...
				this.responsetxt = jQuery.trim(json);
			} catch(_error) {
